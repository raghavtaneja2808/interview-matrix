  ...
  return (
    <div ref={ref} className="relative flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[34px] font-black text-ink leading-none">{value.toFixed(1)}</span>
...
const SkillRow = ({ label, value, max = 10 }) => {
  ...
  return (
    <div ref={ref} className="flex items-center gap-3">
      ...
      <span className="text-[12.5px] font-bold tabular-nums ml-auto shrink-0" style={{ color: barColor }}>
        {value.toFixed(1)}/10
      </span>
    </div>
  );
...
const FOCUS_TIPS = [
  { id: 1, title: "STAR Method", description: "Structure behavioral answers more clearly with specific results." },
  { id: 2, title: "Technical Depth", description: "Elaborate more on system tradeoffs and edge cases when explaining solutions." },
  { id: 3, title: "Reduce Filler Words", description: 'Watch for repeated "basically" and "like" during long explanations.' },
];
...
const Dashboard = () => {
  ...
  const userInfo = dash?.user || {
    ...
    targetRole: authUser?.targetRole || "Frontend Developer",
    lastInterview: "Never",
    level: "Level 3 Candidate",
  };
  const stats = [
    { id: "interviews", icon: "check", label: "INTERVIEWS COMPLETED", value: String(dash?.stats?.totalInterviews ?? 0) },
    { id: "score", icon: "star", label: "AVG SCORE", value: dash?.stats?.avgScore ? dash.stats.avgScore.toFixed(1) : "—" },
    { id: "streak", icon: "flame", label: "CURRENT STREAK", value: `${dash?.stats?.currentStreak ?? 0} day${(dash?.stats?.currentStreak ?? 0) === 1 ? "" : "s"}` },
  ];
  const performance = {
    overall: dash?.performance?.overall ?? 0,
    skills: [
      { label: "Clarity of Speech", value: dash?.performance?.clarity ?? 0 },
      { label: "Confidence Level", value: dash?.performance?.confidence ?? 0 },
      { label: "Technical Understanding", value: dash?.performance?.technical ?? 0 },
    ],
  };
  const recent = dash?.recentInterviews || [];
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <span className="flex items-center gap-1.5 text-[12.5px] text-ink-muted font-medium">
                <CodeChipIcon /> Target: {userInfo.targetRole}
              </span>
              <span className="w-px h-3 bg-border" />
              <span className="flex items-center gap-1.5 text-[12.5px] text-ink-muted font-medium">
                <CalendarIcon /> Last Interview: {userInfo.lastInterview}
              </span>
              <span className="w-px h-3 bg-border" />
              <span className="flex items-center gap-1.5 text-[12.5px] text-ink-muted font-medium">
                <FireIcon /> {userInfo.level}
              </span>
            </div>
          </div>
            ...
            <div className="bg-surface rounded-xl border border-border p-6 flex flex-col">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="flex items-center gap-1 text-[11px] text-green-500 font-semibold">
                  <TrendUpIcon /> {performance.overall >= 7 ? "Improving" : "Building"}
                </span>
              </div>
          ...
          <div className="bg-surface rounded-xl border border-border p-6">
            ...
            {loading ? (
              ...
              <div className="py-10 text-center">
                <p className="text-[14px] text-ink-muted mb-4">No interviews yet. Take your first mock interview to see your performance.</p>
                <button
                  onClick={() => navigate("/dashboard/interview")}
                  className="h-10 px-5 rounded-lg bg-accent hover:bg-accent-dark text-white text-sm font-semibold transition-colors"
                >
              ...
              <>
                ...
                {recent.map((item) => (
                  <button
                    ...
                    className="w-full grid grid-cols-[2fr_1.5fr_1fr_1fr] gap-4 items-center px-3 py-4 border-b border-border-faint last:border-0 hover:bg-surface-muted rounded-lg transition-colors text-left"
                  >
